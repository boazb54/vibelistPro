  ...
  *   🟢 **GOOD SELECTION:** "Faded (Restrung)" by Alan Walker or "Ambient Mix" by similar artists.
  ...
  `;
    ...
    try {
      ...
      console.error(`[API/VIBE] Gemini Error Details: Name=${(geminiError as Error).name}, Message=${(geminiError as Error).message}`);
      // Log the full error object for more details
      console.error("[API/VIBE] Gemini Error Object:", JSON.stringify(geminiError, null, 2));
      if ((geminiError as Error).stack) {
        console.error("[API/VIBE] Gemini Error Stack:", (geminiError as Error).stack);
      }
      throw new Error(`Gemini API Error: ${(geminiError as Error).message || 'Unknown Gemini error'}`);
    }
    ...
    if (geminiResponseText) {
      try {
        ...
        console.error(`[API/VIBE] Parsing Error Details: Name=${(parseError as Error).name}, Message=${(parseError as Error).message}`);
        ...
        return res.status(500).json({ error: `Failed to parse AI response: ${(parseError as Error).message}` });
      }
    }
    ...
    console.error(`[API/VIBE] Uncaught Error Details: Name=${(error as Error).name}, Message=${(error as Error).message}`);
    // Log the full error object for more details
    console.error("[API/VIBE] Uncaught Error Object:", JSON.stringify(error, null, 2));
    if ((error as Error).stack) {
      console.error("[API/VIBE] Uncaught Error Stack:", (error as Error).stack);
    }
