...
const MoodSelector: React.FC<MoodSelectorProps> = ({ onSelectMood, isLoading }) => {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col h-full animate-fade-in-up">
      {/* FIXED TOP SECTION: Header, Input, Ribbon */}
      <div className="flex-none px-4 pt-4 md:pt-8 pb-2">
        
        {/* Header - Compact on Mobile */}
        <div className="text-center mb-2 mt-2 md:mb-6 md:mt-6">
            <h2 className="text-2xl md:text-5xl font-bold mb-1 md:mb-4 bg-clip-text text-transparent bg-gradient-to-r from-cyan-400 to-purple-500">
            How are you feeling?
            </h2>
            <p className="text-slate-400 text-xs md:text-lg max-w-2xl mx-auto hidden md:block">
                ...
                <div className="relative bg-slate-900 rounded-3xl p-1.5">
                    <div className="relative">
                        <textarea
                            ...
                            rows={isRecording ? 3 : 2}
                            className={`w-full bg-slate-800/80 text-white placeholder-slate-400 rounded-2xl py-6 md:py-12 pl-5 pr-12 focus:outline-none resize-none align-top text-base md:text-lg leading-relaxed transition-colors ${isRecording ? 'placeholder-red-400/70 text-red-200' : ''}`}
                        />
                        {/* Voice Input Button */}
                        <button 
                            type="button"
                    ...
                    <div className="flex justify-between items-center px-3 py-2">
                        ...
                        <button
                            ...
                            className="bg-white text-black hover:bg-gray-200 font-bold rounded-xl px-4 py-2 md:px-8 md:py-3 text-xs md:text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-white/20 transform hover:-translate-y-0.5 active:translate-y-0"
                        >
        ...
        {/* DIVIDER */}
        <div className="flex items-center justify-center gap-4 mb-2 md:mb-8 opacity-50 py-2">
            <div className="h-px bg-slate-600 w-16 md:w-24"></div>
            <span className="text-slate-400 text-xs md:text-sm uppercase tracking-widest font-medium">Or choose a quick vibe</span>
            <div className="h-px bg-slate-600 w-16 md:w-24"></div>
        </div>
      </div>

      {/* SCROLLABLE BOTTOM SECTION: Quick Vibe Grid */}
      <div className="flex-1 overflow-y-auto min-h-0 px-4 pb-20 md:pb-4 scroll-smooth">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-4">
            ...
            <button
                ...
                className={`group relative overflow-hidden rounded-xl p-3 md:rounded-2xl md:p-6 transition-all duration-300 hover:scale-105 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed
            ...
            >
                ...
                <div className="relative z-10 flex flex-row md:flex-col items-center justify-center gap-3 md:gap-0">
                    <span className="text-2xl md:text-4xl md:mb-2 drop-shadow-lg transform group-hover:scale-110 transition-transform">{m.emoji}</span>
                    <span className="font-semibold text-white tracking-wide text-sm md:text-base">{m.label}</span>
                </div>
            </button>
            ))}
        </div>
