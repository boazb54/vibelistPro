...
export const createSpotifyPlaylist = async (token: string, playlist: Playlist, userId: string) => {
  ...
  const createRes = await fetch(`https://api.spotify.com/v1/users/${userId}/playlists`, {
    ...
    body: JSON.stringify({
      ...
      public: false, // Ensure private by default as requested
    }),
  });
...
// NEW: Fetch User's Top Artists to build the Taste Profile
export const fetchUserTopArtists = async (token: string): Promise<UserTasteProfile | null> => {
  try {
    ...
    if (!res.ok) {
      console.warn("Failed to fetch top artists, scopes might be missing or user has no data.");
      return null;
    }
    ...
    if (data.items) {
      ...
      // Sort genres by frequency could be better, but slice is fine for now
      const topGenres = uniqueGenres.slice(0, 10) as string[];
