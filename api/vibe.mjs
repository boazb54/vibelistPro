...
export default async function handler(req, res) {
  ...
  // --- API KEY VALIDATION ---
  console.log(`[API/VIBE] DEBUG: Checking process.env.API_KEY. Is it truthy? ${!!process.env.API_KEY}`);
  console.log(`[API/VIBE] DEBUG: API_KEY value (first 5 chars): ${process.env.API_KEY ? String(process.env.API_KEY).substring(0, 5) + '...' : 'undefined'}`);
  if (!process.env.API_KEY) {
    console.error("[API/VIBE] API_KEY environment variable is not set.");
