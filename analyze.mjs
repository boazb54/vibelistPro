  ...
  try {
    ...
    if (type === 'playlists') {
      ...
      try {
        ...
        console.error(`[API/ANALYZE] Gemini Error Details (playlists): Name=${(geminiError as Error).name}, Message=${(geminiError as Error).message}`);
        console.error("[API/ANALYZE] Gemini Error Object (playlists):", JSON.stringify(geminiError, null, 2));
        if ((geminiError as Error).stack) {
          console.error("[API/ANALYZE] Gemini Error Stack (playlists):", (geminiError as Error).stack);
        }
        throw new Error(`Gemini API Error (playlists): ${(geminiError as Error).message || 'Unknown Gemini error'}`);
      }
      ...
      try {
        ...
        console.error(`[API/ANALYZE] Parsing Error Details (playlists): Name=${(parseError as Error).name}, Message=${(parseError as Error).message}`);
        ...
        return res.status(500).json({ error: `Failed to parse AI response for playlist mood: ${(parseError as Error).message}` });
      }
    ...
    Return the result as a raw JSON array.`;
      ...
      try {
        ...
        console.error(`[API/ANALYZE] Gemini Error Details (tracks): Name=${(geminiError as Error).name}, Message=${(geminiError as Error).message}`);
        console.error("[API/ANALYZE] Gemini Error Object (tracks):", JSON.stringify(geminiError, null, 2));
        if ((geminiError as Error).stack) {
          console.error("[API/ANALYZE] Gemini Error Stack (tracks):", (geminiError as Error).stack);
        }
        throw new Error(`Gemini API Error (tracks): ${(geminiError as Error).message || 'Unknown Gemini error'}`);
      }
      ...
      try {
        ...
        console.error(`[API/ANALYZE] Parsing Error Details (tracks): Name=${(parseError as Error).name}, Message=${(parseError as Error).message}`);
        console.error("[API/ANALYZE] Malformed response text (track analysis):", cleanText);

        const t_handler_end = Date.now();
        console.log(`[API/ANALYZE] Handler finished with parsing error in ${t_handler_end - t_handler_start}ms.`);
        return res.status(500).json({ error: `Failed to parse AI response for track analysis: ${(parseError as Error).message}` });
      }
    }
    ...
    console.error(`[API/ANALYZE] Uncaught Error Details: Name=${(error as Error).name}, Message=${(error as Error).message}`);
    console.error("[API/ANALYZE] Uncaught Error Object:", JSON.stringify(error, null, 2));
    if ((error as Error).stack) {
      console.error("[API/ANALYZE] Uncaught Error Stack:", (error as Error).stack);
    }
